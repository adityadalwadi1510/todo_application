...
class HomeState extends State<Home> {
  ...
  ListView getNotesListView() {
    ...
    return ListView.builder(
      itemCount: count,
      itemBuilder: (BuildContext contex, int pos) {
        return Card(
          color: Colors.white,
          elevation: 2.0,
          child: ListTile(
            ...
            trailing: GestureDetector(
              ...
              onTap: () {
                _delete(context, this.noteList[pos]);
              },
            ),
            onTap: () {
              debugPrint("Tapped");
  ...
  void _delete(BuildContext context, Note note) async {
    ...
    if (result != 0) {
      _showShanckBar(context, "Note Deleted Successfully");
      updateListView();
    }
  }

  void _showShanckBar(BuildContext context, String message) {
  ...
  void updateListView() {
    ...
    dbFuture.then((database) {
      Future<List<Note>> noteInFuture = databaseHelper.getNoteList();
      noteInFuture.then((noteList) {
        setState(() {
